use super::aarch64data::{Command, Relocation, SpecialComm, COND_MAP, SPECIAL_IDENT_MAP};
use super::ast::{FlatArg, Modifier, RegId, RegKind};
use super::encoding_helpers;
use super::matching::MatchData;
use super::Context;

use crate::common::{bitmask, delimited, Size, Stmt};
use crate::parse_helpers::{as_float, as_ident, as_number, as_signed_number};
...
pub(super) fn compile_instruction(
    ...
    for command in data.data.commands.iter() {
        ...
        let arg = data
            .args
            .get(cursor)
            .expect("Invalid encoding data, tried to process more arguments than given");

        match *arg {
            FlatArg::Direct {
                span,
                reg: RegKind::Static(id),
            } => {
                match *command {
                    Command::R(offset) => {
                    ...
                    Command::RNext => {
                        if let Some(FlatArg::Direct {
                            span: _prevspan,
                            reg: ref prevreg,
                        }) = data.args.get(cursor - 1)
                        {
                            match prevreg {
                                RegKind::Static(previd) => {
                                    if id.code() != ((previd.code() + 1) % 32) {
            ...
            FlatArg::Direct {
                span,
                reg: RegKind::Dynamic(_, ref expr),
            } => match *command {
                Command::R(offset) | Command::RNoZr(offset) => {
                    dynamics.push((
                        offset,
                        quote_spanned! { span=>
                            #expr & 0x1F
            ...
            FlatArg::Immediate { ref value } => match *command {
                ...
                Command::BUsum(bitlen) => {
                    let prev = if let Some(FlatArg::Immediate { value: leftvalue }) =
                        data.args.get(cursor - 1)
                    {
                        leftvalue
                    } else {
                ...
                Command::Special(offset, special) => {
                    handle_special_immediates(offset, special, value, &mut statics, &mut dynamics)?
                }
            ...
            FlatArg::Default => match *command {
                ...
                Command::BUbits(_) | Command::BSscaled(_, _) => (),
        ...
        match *command {
            Command::Uslice(_, _, _) | Command::Sslice(_, _, _) => (),
            Command::BUbits(_)
            | Command::BUsum(_)
    ...
    if !dynamics.is_empty() {
        ...
        ctx.state
            .stmts
            .push(Stmt::ExprUnsigned(delimited(res), Size::DWORD));
    } else {
        ctx.state
            .stmts
            .push(Stmt::Const(u64::from(bits), Size::DWORD));
    }
...
fn handle_special_immediates(
    ...
    dynamics: &mut Vec<(u8, TokenStream)>,
) -> Result<(), Option<String>> {
    match special {
        SpecialComm::INVERTED_WIDE_IMMEDIATE_X => {
            if let Some(number) = as_number(imm) {
        ...
        SpecialComm::INVERTED_WIDE_IMMEDIATE_W => {
            if let Some(number) = as_number(imm) {
                if number <= u64::from(std::u32::MAX) {
                    if let Some(encoded) =
                        encoding_helpers::encode_wide_immediate_32bit(!(number as u32))
                    {
                        statics.push((offset, encoded as u32));
                        return Ok(());
                    }
        ...
        SpecialComm::STRETCHED_IMMEDIATE => {
            if let Some(number) = as_number(imm) {
        ...
        SpecialComm::LOGICAL_IMMEDIATE_W => {
            if let Some(number) = as_number(imm) {
                if number <= u64::from(std::u32::MAX) {
                    if let Some(encoded) =
                        encoding_helpers::encode_logical_immediate_32bit(number as u32)
                    {
        ...
        SpecialComm::LOGICAL_IMMEDIATE_X => {
            if let Some(number) = as_number(imm) {
        ...
        SpecialComm::FLOAT_IMMEDIATE => {
            if let Some(number) = as_float(imm) {
                if let Some(encoded) =
                    encoding_helpers::encode_floating_point_immediate(number as f32)
                {
                    statics.push((offset, u32::from(encoded)));
                    return Ok(());
                }
        ...
        SpecialComm::SPLIT_FLOAT_IMMEDIATE => {
            if let Some(number) = as_float(imm) {
                if let Some(encoded) =
                    encoding_helpers::encode_floating_point_immediate(number as f32)
                {
...
fn unsigned_rangecheck(
    expr: &syn::Expr,
    min: u32,
    max: u32,
    scale: u8,
) -> Option<Result<u32, Option<String>>> {
