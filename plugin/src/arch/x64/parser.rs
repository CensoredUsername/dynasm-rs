...
use crate::parse_helpers::{
    as_ident, eat_pseudo_keyword, parse_ident_or_rust_keyword, ParseOptExt,
};

use super::ast::{Instruction, MemoryRefItem, RawArg, RegFamily, RegId, Register};
...
fn parse_arg(ctx: &mut Context, input: parse::ParseStream) -> parse::Result<RawArg> {
    ...
    if let Some(jump) = input.parse_opt()? {
        return Ok(RawArg::JumpTarget { jump, size });
    }
    ...
    if input.peek(syn::token::Bracket) {
        ...
        if let Some(jump) = inner.parse_opt()? {
            return Ok(RawArg::IndirectJumpTarget { jump, size });
        }
    ...
    if let Some((span, reg)) = parse_reg(ctx, &arg) {
        ...
        return Ok(RawArg::Direct { reg, span });
    }

    // immediate
    Ok(RawArg::Immediate { value: arg, size })
}
...
fn parse_reg(ctx: &Context, expr: &syn::Expr) -> Option<(Span, Register)> {
    if let Some(path) = as_ident(expr) {
        ...
        Some((path.span(), Register::new_static(size, reg)))
    } else if let syn::Expr::Call(syn::ExprCall {
        ref func, ref args, ..
    }) = expr
    {
...
fn collect_adds(node: syn::Expr, collection: &mut Vec<syn::Expr>) {
    ...
    } else if let syn::Expr::Binary(syn::ExprBinary {
        op: syn::BinOp::Sub(sub),
        left,
        right,
        ..
    }) = node
    {
        collect_adds(*left, collection);
        collection.push(syn::Expr::Unary(syn::ExprUnary {
            op: syn::UnOp::Neg(sub),
            expr: right,
            attrs: Vec::new(),
        }));
